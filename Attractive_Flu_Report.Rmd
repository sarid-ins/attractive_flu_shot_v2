...
# compare spammers versus time to complete

complete_duration %>% 
  left_join(open_ended) %>% 
  count(spam, duration_groups)

flu_data %>% 
  left_join(complete_duration) %>% 
  filter(duration <= 30) 
  

```

## Certainty Level of Late Vaccinations

```{r certaint of late vaccinations}

flu_data %>% 
  filter(str_detect(action_recoded, "late")) %>% 
  saridr::prop(certainty, leave_n = T)

```

## Comparison Type Open Ended Comments versus Treatment

```{r comparison comments vs treatment}
comparison_prop_tbl <- flu_data %>% 
  left_join(open_ended) %>% 
  mutate(Uncertain = certainty <= 3) %>% 
  group_by(treatment, Uncertain) %>% 
  summarize(portion_comparison = mean(comparison)) %>% 
  pivot_wider(id_cols = Uncertain, names_from = treatment, values_from = portion_comparison) %>% 
  select(Uncertain, Control, Recommendation, Cost, Benefit, Stock)

comparison_abs_str <- flu_data %>% 
  left_join(open_ended) %>% 
  mutate(Uncertain = certainty <= 3) %>% 
  group_by(treatment, Uncertain) %>% 
  summarize(tot_comparison = sum(comparison),
            out_of = n()) %>% 
  mutate(out_of_str = paste0("(", tot_comparison, "/", out_of, ")")) %>% 
  select(1,2,5) %>% 
  pivot_wider(id_cols = Uncertain, names_from = treatment, values_from = out_of_str) %>% 
  select(Uncertain, Control, Recommendation, Cost, Benefit, Stock)

```

```{r comparative and certainty}

flu_data %>% 
  left_join(open_ended) %>% 
  mutate(Uncertain = certainty <= 3) %>% 
  group_by(Uncertain, comparison) %>% 
  summarize(intentions_to_vaccinate = mean(action_binary))



comparison_prop_tbl <- flu_data %>%
  filter(action_binary == 1) %>% 
  left_join(open_ended) %>% 
  mutate(Uncertain = certainty <= 3) %>% 
  group_by(treatment, Uncertain) %>% 
  summarize(portion_comparison = mean(comparison)) %>% 
