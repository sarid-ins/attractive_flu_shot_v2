...
# Using Chi square test to examine the independence of the intervention vs. intentions to vaccinate
...
p.adjust(p = c(chisq1$p.value, chisq2$p.value), method = "fdr")
...
vaccination_glm1 <- glm(data = flu_data_for_glm, 
                        family = binomial, 
                        formula = action_binary ~ treatment + certain + treatment*certain)
...
vaccination_glm2 <- glm(data = flu_data_for_glm, family = binomial, 
                        formula = action_binary ~ 
                          treatment + certain + treatment*certain + 
...
# Additional requests for supplementary materials/appendix

A comparison table per certainty level (like table 3 but without the grouping of certainty to just two levels)

```{r comparison per certainty level 1-5}
flu_data %>% 
  group_by(treatment, certainty) %>% 
  summarize(prop_vaccinated = mean(action_binary),
            vaccinated_abs = sum(action_binary),
...
The Mann-Witney-Wilcoxon test per certainty level

```{r mww per certainty level}
provide_p_value_MWW_certainty <- function(data, treatment_filter, certainty_filter){
  tmp <- data %>% 
    filter(certainty == certainty_filter) %>% 
    filter(treatment %in% c("Control", treatment_filter)) %>% 
    wilcox.test(formula = action_binary ~ treatment, data = .)
...
crossing(treatments = c("Recommendation", "Stock", "Cost", "Benefit"),
         certainty = 1:5) %>% 
  mutate(p.values = map2_dbl(treatments, certainty,
                             ~{provide_p_value_MWW_certainty(data = flu_data, 
                                                             treatment_filter = .x, 
                                                             certainty_filter =.y)})) %>% 
  mutate(fdr_adjusted = p.adjust(p.values, method = "fdr")) %>% 
...
## Effect of monetary incentives as a function of income levels
...
provide_p_value_chisq_income <- function(data, treatment_filter, income_filter){
  tmp <- data %>% 
    filter(income == income_filter) %>% 
    filter(treatment %in% c("Control", treatment_filter)) %>% 
    count(income, treatment, action_binary) %>% 
    select(-income) %>% 
  ...
  tibble(p.value = tmp$p.value, chisq = tmp$statistic)
}


crossing(treatments = c("Recommendation", "Stock", "Cost", "Benefit"),
         income = c("Under", "Average", "Above")) %>% 
  mutate(p.values = map2(treatments, income,
                             ~{provide_p_value_chisq_income(data = flu_data, 
                                                             treatment_filter = .x, 
                                                             income_filter =.y)})) %>% 
  unnest() %>% 
  mutate(fdr_adjusted = p.adjust(p.value, method = "fdr")) %>% 
  knitr::kable(format = "latex", booktabs = TRUE)
...
chi_goodness_income <- tibble(income_lvl = c("Under", "Average", "Above")) %>% 
  ...
  unnest(chi_sq) %>% 
  mutate(p_fd_adj = p.adjust(p_val, "fdr"))
...
certainty_chisq <- list(1,2,3,4,5, 1:5, 1:3, 4:5) %>% 
  map_df(.f = get_chisq_pval) %>% 
  mutate(p.value.fdr = p.adjust(p.val, method = "fdr")) %>% 
  mutate(certainty_level = as.character(list(1,2,3,4,5, 1:5, 1:3, 4:5)))

```

# Proportion test for the effect of each treatment versus control

```{r proportion test}

compute_prop_test <- function(treatment_str, certainty_lvl){
  tmp <- flu_data %>% 
    filter(treatment %in% c("Control", treatment_str)) %>% 
    filter(certainty %in% certainty_lvl) %>% 
    group_by(treatment) %>% 
    summarize(successes = sum(action_binary),
              total = n())
  
  binom.test(tmp$successes, tmp$total)
  
}

run_prop_tests <- crossing(certainty_levels = list(1,2,3,4,5, 1:3, 4:5, 1:5),
                           treatments = c("Recommendation", "Stock", "Cost", "Benefit")) %>% 
  mutate(test_res = map2(treatments, certainty_levels, compute_prop_test)) %>% 
  mutate(test_pval = map_dbl(test_res, ~{.$p.value})) %>% 
  mutate(certainty_levels = as.character(certainty_levels)) %>% 
  select(-test_res) %>% 
  mutate(test_pval_fdr = p.adjust(test_pval, "fdr"))

# knitr::kable(run_prop_tests, format = "markdown")
