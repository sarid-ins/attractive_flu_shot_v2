...
# instead of five year vaccination - also check last year vaccination

vaccination_glm1_mod <- glm(data = flu_data_for_glm, 
                        family = binomial, 
                        formula = action_binary ~ treatment + last_year_vaccination + treatment*last_year_vaccination)

summary(vaccination_glm1_mod)

```

We would like to compare the difference between the Cost treatment coefficient to the Recommendation treatment coefficient, in order to see if the cost is significantly stronger than the recommendation (within the regression model).

```{r coefficient comparison}

# recommendation - cost
rec_min_cost <- vaccination_glm1$coefficients[2] - vaccination_glm1$coefficients[4]

rec_min_cost
# var(recommendation - cost) = var(recommendation) + var(cost) - 2*cov(recommendation, cost)

vcov_glm1 <- vcov(vaccination_glm1)
rec_min_cost_std_err <- sqrt(
  vcov_glm1[2, 2] + vcov_glm1[4, 4] - 2*vcov_glm1[2, 4]
)
