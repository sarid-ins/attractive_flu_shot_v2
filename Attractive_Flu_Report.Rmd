...
provide_p_value_MWW_income <- function(data, treatment_filter, income_filter){
  tmp <- data %>% 
...
crossing(treatments = c("Recommendation", "Stock", "Cost", "Benefit"),
  ...
  mutate(p.values = map2_dbl(treatments, income,
                             ~{provide_p_value_MWW_income(data = flu_data, 
                                                             treatment_filter = .x, 
                                                             income_filter =.y)})) %>% 
  mutate(fdr_adjusted = p.adjust(p.values, method = "fdr")) %>% 
  knitr::kable(format = "latex", booktabs = TRUE)
