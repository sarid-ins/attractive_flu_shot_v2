...
comparison_prop_tbl <- flu_data %>% 
  left_join(open_ended) %>% 
  mutate(Certainty = certainty <= 3) %>% 
  group_by(treatment, Certainty) %>% 
  summarize(portion_comparison = mean(comparison)) %>% 
  pivot_wider(id_cols = Certainty, names_from = treatment, values_from = portion_comparison) %>% 
  select(Certainty, Control, Recommendation, Cost, Benefit, Stock)
...
comparison_abs_str <- flu_data %>% 
  ...
  select(1,2,5) %>% 
  pivot_wider(id_cols = Certainty, names_from = treatment, values_from = out_of_str) %>% 
  select(Certainty, Control, Recommendation, Cost, Benefit, Stock)
